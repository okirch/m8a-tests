...
class GitRepositoryBuild(ToParamMixin):
    ...
    the dockerfiles/ subfolder. This class is to be used in conjunction with
    the dockerfile_build fixture.

    Each build corresponds to a file with the filename being equal to the
    `repo_name` property, that is copied by the `dockerfile_build` into a
    temporary working directory as `Dockerfile`.
    """
